  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        const apiKey = import.meta.env.VITE_FILLFEEDBACK_API_KEY || "fv_pub_0ogMfPvLtLRQXA0MHU2BvaAid2I0QMnl-fAMMyZa";
        
        const response = await fetch("https://api.fillfeedback.com/api/v1/reviews", {
          method: "GET",
          headers: {
        ...
        if (result.success && result.data && result.data.data) {
          ...
          const validReviews = result.data.data
            .filter((rev: FillFeedbackReview) => rev.rating >= 4 && rev.answers?.Review)
            .slice(0, 3);
